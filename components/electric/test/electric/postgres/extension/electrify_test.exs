defmodule Electric.Postgres.Extension.ElectrifyTest do
  ...
  test_tx "inserts a row into the electrified table", fn conn ->
    sql = "CREATE TABLE buttercup (id uuid PRIMARY KEY DEFAULT gen_random_uuid());"
  ...
  test_tx "handles quoted table names", fn conn ->
    sql = "CREATE TABLE \"la la daisy\" (id uuid PRIMARY KEY DEFAULT gen_random_uuid());"
    {:ok, _cols, _rows} = :epgsql.squery(conn, sql)

    sql = "CREATE TABLE \"la la buttercup\" (id uuid PRIMARY KEY DEFAULT gen_random_uuid());"
    {:ok, _cols, _rows} = :epgsql.squery(conn, sql)
  ...
  test_tx "allows for namespaced table names", fn conn ->
    sql = "CREATE TABLE daisy (id uuid PRIMARY KEY DEFAULT gen_random_uuid());"
    {:ok, _cols, _rows} = :epgsql.squery(conn, sql)
  ...
  test_tx "adds the electrified table to the publication", fn conn ->
    ...
    sql = "CREATE TABLE buttercup (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), value text);"
